...
const postsBase = new Airtable({ apiKey: process.env.AIRTABLE_API_KEY }).base(
  process.env.AIRTABLE_POSTS_BASE_ID
)
...
function lookupAuthor(name) {
  const nameQuery = name.split(/\s/).map(toSentenceCase).join(" ")
  ...
  return new Promise((resolve, error) => {
    postsBase("Posts").create(
      ...
      function (err, record) {
        ...
        console.log(
          `${chalk.greenBright("Success!")} Entry ${record.getId()} created`
        )
        /* TODO: Actually create the page stub? */
        console.log(
          `${chalk.yellowBright(
            "Don't forget to create:"
          )} /src/news/${postSlug}.md`
        )
      }
    )
  })
...
async function askQuestions() {
  ...
  var rawTitle = await interactPrompt(
    "Give your post a title. (You can change it later.)\n"
  )
  ...
  console.log(`
  ${chalk.blueBright(
    "Based on your responses, we'll create the following post entry:"
  )}

...
const ResearchIndex = ({data}: Props) => {
  const items = data.allAirtableResearchItems.nodes
  const sortedItems = Array.from(items).sort((a: Queries.ResearchQuery["allAirtableResearchItems"]["nodes"][number]) => (a?.data?.active === "TRUE" ? -1 : 1))

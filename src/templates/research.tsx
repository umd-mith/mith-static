...
const Research = ({ pageContext: item }: Props) => {
  ...
  if (item.participants) {
    participant_list = item.participants.map((p, i) => {
      return <Person key={`p${i}`} person={p.data as unknown as PersonComponentProps} type="participant" />
    })
  ...
  if (item.directors) {
    ...
    director_list = item.directors.map(person => {
      return <Person person={person.data as unknown as PersonComponentProps} type="director" />
    })
  ...
  if (item.twitter_account) {
    ...
    //   </span> : null
    twitter = <div className="twitter">{iconTwitter} {twitter_acct}</div>
  }
  ...
  if (item.linked_links) {
    links_list = item.linked_links.map(_l => {
      ...
      link_url = l.url?.startsWith('http') 
        ? l.url 
        : `http://${l.url}`
      return <li id={l.title || ""}><a href={link_url} title={l.title || ""} target="_blank" rel="noreferrer">{l.title}</a></li>
    })
  ...
  const sponsors = item.linked_sponsors && item.linked_sponsors.length > 0 
    ? <SupporterList supporters={item.linked_sponsors as SupporterComponentProps} type="sponsor" />
    : ''
  const partners = item.linked_partners && item.linked_partners.length > 0 
    ? <SupporterList supporters={item.linked_partners as SupporterComponentProps} type="partner" />
    : ''
  ...
  if (item.linked_events) {
    events_list = item.linked_events.map(_e => {
      ...
      return <li id={e.id!} className="event">
        ...
        <EventTime start={parseInt(e.start_date!)} end={parseInt(e.end_date!)} />
  ...
  if (item.linked_posts) {
    news_list = item.linked_posts.map(_n => {
      ...
      return <li id={n.slug?.toLowerCase().replace(/-/g, '_')}>
        <div className="post-title"><Link key={`n-${n.slug || ""}`} to={`../../news/${n.slug}`}>{n.post_title}</Link></div>
        <div className="meta">

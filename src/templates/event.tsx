...
interface Props {
  pageContext: NonNullable<Queries.PageEventQuery["allAirtableEvents"]["nodes"][number]["data"]>
}

const Event = ({ pageContext: item } : Props) => {
  ...
  if (speakers_data.length > 0) {
    speakers_list = speakers_data.map((p, i) => {
      return <Person key={`p${i}`} person={p?.data as unknown as PersonComponentProps} type="speaker" />
    })
  ...
  if (item.linked_research_item) {
    ...
    research = <div className="related-research">
